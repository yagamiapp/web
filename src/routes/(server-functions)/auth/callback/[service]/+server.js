...
export async function GET({ url, params, cookies }) {
	...
	if (params.service == 'osu') {
		...
		let { current: level, progress: level_progress } = userResponse.statistics.level;
	...
	if (params.service == 'discord') {
		...
		let tokenRequestForm = new URLSearchParams(Object.entries(tokenRequest)).toString();
		...
		if (!accountTest || !accountTest.includes(id)) {
			let { access_token, expires_in, refresh_token, token_type, scope } = response;
